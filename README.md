...
For every `.docx` in `1_Inbox` the pipeline downloads it, runs the formatter
and then:
...
so on. Non-`.docx` files in the inbox are left alone. Because processed
originals always leave the inbox, re-running is safe.
